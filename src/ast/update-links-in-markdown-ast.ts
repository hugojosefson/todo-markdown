 ...
 * This function does not use {@link _transformNode}, because it does not need to transform the AST in that way. Instead, it traverses the AST, and updates all links in the AST.
...
export function updateLinksInMarkdownAst<T extends Nodes>(
  ...
  if (isParent(ast)) {
    const children: Parent["children"] = ast.children.map((child) =>
